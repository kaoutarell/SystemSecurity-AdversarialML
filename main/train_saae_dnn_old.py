...
def main():
    ...
    print("\n" + "="*80)
    print("Step 4: Evaluation on KDDTest+")
    print("="*80)
    
    # Determine if binary or multiclass
    is_binary = (num_classes == 2) or (args.binary)
    
    # Evaluate on KDDTest+
    metrics_test = evaluate_model(
        saae_dnn_model, X_test, y_test, num_classes=num_classes, binary=is_binary
    )
    
    # Save evaluation results
    with open(run_dir / 'metrics_kddtest+.json', 'w') as f:
        json.dump(metrics_test, f, indent=2)
    
    # Get predictions for confusion matrix
    y_pred_probs = saae_dnn_model.predict(X_test, verbose=0)
    if is_binary or num_classes == 2:
    ...
    plot_confusion_matrix(y_true, y_pred, run_dir / 'confusion_matrix_kddtest+.png')
    ...
    if args.test_21_path:
        ...
        #     saae_dnn_model, X_test_21, y_test_21, num_classes=num_classes, binary=is_binary
        # )
