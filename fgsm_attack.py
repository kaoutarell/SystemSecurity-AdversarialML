...
def evaluate_robustness(model, X_clean, y_clean, epsilons):
    ...
    print("="*70)
    probs_clean, preds_clean = model.predict(X_clean, verbose=0), (model.predict(X_clean, verbose=0) >= 0.5).astype(int).flatten()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='models/nn_ids_model.keras',
                        help='Path to trained model')
    parser.add_argument('--artifacts', type=str, default='models/artifacts.joblib',
    ...
    parser.add_argument('--epsilons', type=float, nargs='+', default=[0.01, 0.02, 0.05, 0.1, 0.2,0.3],
    ...
    ids = IDSModel(model_path=args.model_path, artifacts_path=args.artifacts).load()
    print('✓ Model and artifacts loaded successfully.')
    
    # Load and preprocess test data
    print(f'\nLoading test data from {args.test_path}')
    X_test, indices, features, y_test = ids.preprocess_file(args.test_path, n_samples=args.n_samples)
    
    if y_test is None:
        raise ValueError("Could not extract labels from test data. Ensure outcome column is present in NSL-KDD file.")
    ...
    results = evaluate_robustness(ids.model, X_test, y_test, args.epsilons)
